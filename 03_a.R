...
# tous les régimes alimentaires simultanément 

nutri_new %>% 
  filter((code_pays=="FRP") & (nutri_new$item=="%rec")) %>% 
  ggplot() +
  aes(fct_reorder(diet.scenario, protein), protein, fill = grp_diet, group = grp_diet) +
  geom_bar(stat= "identity", position = position_dodge())+
  geom_text(aes(y = protein, 
                label = round(protein,0),
                group =grp_diet), 
            color = "black", 
            size = 4,
            vjust = 1) +
